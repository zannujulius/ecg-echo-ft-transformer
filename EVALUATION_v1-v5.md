...
## 2. What was done _right_ (keep these)
...
- v5 applies `class_weight=balanced` **and** focal loss simultaneously. The README calls this the "alpha-balanced variant," but the focal implementation has **no alpha term** — the balancing is entirely Keras `class_weight`. So minority classes are up-weighted by `class_weight` _and_ by the focal modulation. This is double pressure toward minority classes and is exactly why `normal` recall drops to 71% (796 normals predicted as mild) and minority precision is poor.
...
1. **Establish a noise floor.** Re-run v4 and v5 with 5 seeds each; report mean ± std. Confirm whether any post-v2 change is real. _(One afternoon; reframes everything.)_
...
5. **Stop stacking rebalancers** — choose focal-with-alpha _or_ class_weight, then tune γ and thresholds.
...
7. **Only then** do the planned v6 Optuna search — and search the _architecture_ (embedding_dim, blocks, heads, LR), which has never been explored, not just the loss.
